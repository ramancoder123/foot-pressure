...
def calculate_pressure_data():
    ...
    csv_file_path = r"C:\Users\js838\Desktop\GRAPH\data.csv"

    try:
        with open(log_file_path, 'r') as log_file:
            lines = log_file.readlines()

        # Remove the unwanted lines based on the line indices
        ...
        for line in filtered_lines:
            columns = line.split()[1:]  # Skip the first item
        ...
        # Extract the max values for left and right foot pressure calculations
        temp_max = []
        diff_val = []
        ...
        # Left and right foot pressure values
        left_foot = diff_val[5:]
        right_foot = diff_val[:5]

        # Calculate the mean pressures
...
def generate_graph():
    try:
        # Load data from CSV
        df = pd.read_csv(r"C:\Users\js838\Desktop\GRAPH\data.csv")
        ...
        # Calculate the maximum values for channels 1 and 2
        for i in range(1, 6):
            temp = "Ch01Gr0" + str(i)
            maxim = max(df[temp])
            temp_max.append(maxim)
        ...
        # Calculate the left and right foot pressures
